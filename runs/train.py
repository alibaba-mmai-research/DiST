...
def train_epoch(
    train_loader, model, model_ema, optimizer, train_meter, cur_epoch, mixup_fn, cfg, text_tokenizers
):
    """
    ...
    for cur_iter, (inputs, labels, indexes, meta) in enumerate(train_loader):
        ...
        if text_tokenizers is not None:
            inputs['texts'] = text_tokenizers
...
@torch.no_grad()
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, text_tokenizers):
    """
...
def train(cfg):
    ...
    if cfg.DATA.DATASET_LABEL_TEXT.ENABLE:
        train_text_tokenizers = train_loader.dataset.text_tokenizers.cuda(non_blocking=True)
        val_text_tokenizers = val_loader.dataset.text_tokenizers.cuda(non_blocking=True)
    else:
        train_text_tokenizers = None
        val_text_tokenizers = None
    ...
    for cur_epoch in range(start_epoch, cfg.OPTIMIZER.MAX_EPOCH, cfg.TRAIN.NUM_FOLDS):
        ...
        # Train for one epoch.
        train_epoch(
            train_loader, model, model_ema, optimizer, train_meter, cur_epoch, mixup_fn, cfg, text_tokenizers=train_text_tokenizers
        )
        ...
        if misc.is_eval_epoch(cfg, cur_epoch+cfg.TRAIN.NUM_FOLDS-1):
            ...
            eval_epoch(val_loader, model, val_meter, cur_epoch+cfg.TRAIN.NUM_FOLDS-1, cfg, text_tokenizers=val_text_tokenizers)
            if model_ema is not None:
