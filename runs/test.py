...
@torch.no_grad()
def perform_test(test_loader, model, test_meter, cfg, text_tokenizers):
    """
...
def test(cfg):
    ...
    if cfg.DATA.DATASET_LABEL_TEXT.ENABLE:
        test_text_tokenizers = test_loader.dataset.text_tokenizers.cuda(non_blocking=True)
    else:
        test_text_tokenizers = None
    ...
    perform_test(test_loader, model, test_meter, cfg, text_tokenizers=test_text_tokenizers)
    if model_ema is not None:
        test_meter.set_model_ema_enabled(True)
        perform_test(test_loader, model_ema.module, test_meter, cfg, text_tokenizers=test_text_tokenizers)
